...
class RelayStoryService(commands.Cog):
    ...
    async def _import_legacy_story(self):
        guild_id, channel_id = os.getenv("SECRET_ROOM_SERVER_ID"), os.getenv("RELAY_STORY_ID")
        if not (guild_id and channel_id and guild_id.isdecimal() and channel_id.isdecimal()):
            return
        ...
        for reward in await self.bot.db.fetch("SELECT message_id,discord_id,awarded_at FROM si_relay_rewards"):
            await self.bot.db.execute(
                ...
                story["id"], reward["message_id"], reward["discord_id"], reward["awarded_at"],
            )

    async def create_story(self, guild_id: int, name: str, channel_id: int, rules_text: str | None = None):
        ...
        return await self.bot.db.fetchrow(
            ...
            guild_id, key, name, channel_id, rules_text,
        )
    ...
    async def validate_story_channel(self, guild_id: int, channel_id: int) -> None:
        ...
        if getattr(getattr(channel, "guild", None), "id", None) != guild_id or not hasattr(channel, "permissions_for"):
        ...
        if not (permissions.view_channel and permissions.send_messages and permissions.read_message_history):
            raise ValueError("The bot needs view, send, and message-history permissions in the relay story channel.")

    async def legacy_mismatches(self, guild_id: int) -> list[str]:
        """Compare the legacy relay import and its reward evidence."""
        configured_guild, configured_channel = os.getenv("SECRET_ROOM_SERVER_ID"), os.getenv("RELAY_STORY_ID")
        if not (configured_guild and configured_channel and configured_guild.isdecimal() and configured_channel.isdecimal()):
            return []
        ...
        legacy_count = await self.bot.db.fetchval("SELECT COUNT(*) FROM si_relay_rewards")
        ...
        if legacy_count != imported_count:
            mismatches.append(f"relay: 보상 기록 수 불일치 ({imported_count}/{legacy_count})")
        return mismatches

    async def pin_rules(self, story_id: int):
        story = await self.bot.db.fetchrow("SELECT * FROM relay_stories WHERE id=$1", story_id)
        if story is None or not story["rules_text"]:
            raise ValueError("고정할 이야기 규칙이 없습니다.")
        channel = self.bot.get_channel(story["channel_id"]) or await self.bot.fetch_channel(story["channel_id"])
    ...
    async def on_message(self, message: discord.Message):
        ...
        contribution = await self.bot.db.fetchrow(
            ...
            story["id"], message.id, message.author.id,
            "pending" if story["reward_policy"].get("type") != "none" else "not_applicable",
        )
        if contribution:
    ...
    async def _reject_message(self, message: discord.Message, story, reason: str):
        try:
            ...
            return
        try:
            locale = await guild_locale(self.bot.db, story["guild_id"], message.guild.preferred_locale)
            await message.author.send(text("relay.contribution_deleted", locale, name=story["display_name"], reason=reason))
        except discord.HTTPException:
            pass
        await self.bot.db.execute(
            ...
            story["guild_id"], message.author.id, str(story["id"]), {"message_id": message.id, "reason": reason},
        )

    async def _issue_legacy_reward(self, story, contribution, message):
        if not await game_enabled(self.bot.db, story["guild_id"]):
            await self.bot.db.execute("UPDATE relay_contributions SET reward_state='not_applicable' WHERE id=$1", contribution["id"])
            return
        ...
        try:
            if policy_type == "legacy_secret_multiplier":
                ...
                reward = await grant_relay_secret_reward(self.bot.db, story["guild_id"], message.author.id, message.id)
            else:
                from .secretae_incremental.db import grant_configured_reward
                reward = await grant_configured_reward(self.bot.db, story["guild_id"], "relay_story", story["id"], message.id, message.author.id, story["reward_policy"])
            state = "granted" if reward.get("awarded") or reward.get("already_awarded") else ("not_applicable" if reward.get("cap_reached") else "failed")
            await self.bot.db.execute("UPDATE relay_contributions SET reward_state=$2,rewarded_at=NOW() WHERE id=$1", contribution["id"], state)
            if reward.get("awarded") and policy_type == "legacy_secret_multiplier":
                locale = await guild_locale(self.bot.db, story["guild_id"], message.guild.preferred_locale)
                lines = [text("relay.legacy_reward.title", locale)]
                for key in SECRETS:
                    lines.append(text(
                        "relay.legacy_reward.line", locale,
                        symbol=SYMBOLS[key],
            ...
            await self.bot.db.execute("UPDATE relay_contributions SET reward_state='failed' WHERE id=$1", contribution["id"])
            await self.bot.db.execute("INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,0,'relay_reward_failed','relay_contribution',$2,$3)", story["guild_id"], str(contribution["id"]), {"error": type(exc).__name__})
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        if row:
            story = await self.bot.db.fetchrow("SELECT guild_id FROM relay_stories WHERE id=$1", row["story_id"])
            from .secretae_incremental.db import register_reward_reconciliation
            reconciliation = await register_reward_reconciliation(self.bot.db, story["guild_id"], "relay_story", payload.message_id, "source message deleted")
            await self.bot.db.execute("INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,$2,'relay_contribution_withdrawn','relay_contribution',$3,$4)", story["guild_id"], row["author_id"], str(payload.message_id), {"reason": "source message deleted"})
            if reconciliation:
                await self.bot.db.execute("INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,0,'reward_reconciliation_required','relay_contribution',$2,$3)", story["guild_id"], str(payload.message_id), reconciliation)
