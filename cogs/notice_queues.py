...
class NoticeQueueService(commands.Cog):
    ...
    async def _import_legacy_queues(self):
        ...
        for (
            ...
            if not (
                source
                and destination
                and source.isdecimal()
                and destination.isdecimal()
            ):
                continue
            ...
            row = await self.bot.db.fetchrow(
                ...
                key,
                name,
                "active" if enabled is not False else "paused",
                int(source),
                int(destination),
                cadence,
                weekday,
    ...
    async def _import_legacy_submissions(self, queue_id: int, kind: str):
        ...
        for row in rows:
            ...
            reward_state = (
                "granted"
                if row["rewarded_at"]
                else ("pending" if row["posted_at"] else "not_applicable")
            )
            await self.bot.db.execute(
    ...
    async def legacy_mismatches(self, guild_id: int) -> list[str]:
        ...
        for key, source_env, target_env, cadence, hour, minute in checks:
            ...
            if not (source and target and source.isdecimal() and target.isdecimal()):
                continue
            row = await self.bot.db.fetchrow(
                "SELECT * FROM notice_queues WHERE guild_id=$1 AND queue_key=$2",
                guild_id,
                key,
            )
            if row is None:
                mismatches.append(f"{key}: 가져온 대기열 없음")
                continue
            if (
                row["submission_channel_id"],
                row["publication_channel_id"],
                row["cadence"],
                row["schedule_time"].hour,
                row["schedule_time"].minute,
            ) != (int(source), int(target), cadence, hour, minute):
    ...
    async def create_queue(
        ...
        if cadence == "weekly" and weekday is None:
            raise ValueError(
                "A weekly queue needs a weekday (0=Monday through 6=Sunday)."
            )
    ...
    async def _validate_channel(self, guild_id: int, channel_id: int, purpose: str):
        ...
        if getattr(channel.guild, "id", None) != guild_id or not hasattr(
        ...
        if not required:
            raise ValueError(
                "봇에 채널 보기, 메시지 보내기, 기록 보기 및 필요한 스레드 생성 권한이 없습니다."
            )

    async def _alert(self, guild_id: int, content: str):
        audit_id = await self.bot.db.fetchval(
            "SELECT audit_channel_id FROM guild_settings WHERE guild_id=$1", guild_id
        )
        if not audit_id:
            return
    ...
    async def open_submissions(self, queue_id: int):
        queue = await self.bot.db.fetchrow(
            "SELECT * FROM notice_queues WHERE id=$1", queue_id
        )
        if queue is None:
            raise ValueError("Queue not found.")
        await self._validate_channel(
            queue["guild_id"], queue["submission_channel_id"], "submission"
        )
        channel = self.bot.get_channel(
            queue["submission_channel_id"]
        ) or await self.bot.fetch_channel(queue["submission_channel_id"])
        if not hasattr(channel, "create_thread"):
            raise ValueError("The submission destination cannot create a thread.")
        created = await channel.create_thread(
            name=f"{queue['display_name']} submissions",
            content=f"Submit an entry for **{queue['display_name']}** here.",
        )
        thread = created.thread if hasattr(created, "thread") else created
        await self.bot.db.execute(
            "UPDATE notice_queues SET active_submission_thread_id=$2, submission_opened_at=NOW(), accepting_submissions=TRUE, updated_at=NOW() WHERE id=$1",
            queue_id,
            thread.id,
        )
        return thread
    ...
    async def on_message(self, message: discord.Message):
        ...
        if queue["submission_period_hours"] and queue["submission_opened_at"]:
            closes_at = queue["submission_opened_at"] + timedelta(
                hours=queue["submission_period_hours"]
            )
            if datetime.now(timezone.utc) >= closes_at:
                await self.bot.db.execute(
                    "UPDATE notice_queues SET accepting_submissions=FALSE,updated_at=NOW() WHERE id=$1",
                    queue["id"],
                )
                await message.delete()
                try:
        ...
        invalid_reason = self._invalid_submission_reason(
            queue, message.content, bool(message.attachments), locale
        )
        if invalid_reason:
            await message.delete()
            try:
    ...
    @staticmethod
    def _invalid_submission_reason(
        queue, content: str, has_attachments: bool, locale="ko"
    ) -> str | None:
        length = len(content.strip())
        if (
            queue["min_content_length"] is not None
            and length < queue["min_content_length"]
        ):
            return text(
                "queue.reason.min_length", locale, length=queue["min_content_length"]
            )
        if (
            queue["max_content_length"] is not None
            and length > queue["max_content_length"]
        ):
            return text(
                "queue.reason.max_length", locale, length=queue["max_content_length"]
            )
    ...
    async def on_raw_message_edit(self, payload: discord.RawMessageUpdateEvent):
        row = await self.bot.db.fetchrow(
            "SELECT s.id,q.* FROM notice_submissions s JOIN notice_queues q ON q.id=s.queue_id WHERE s.source_message_id=$1 AND s.status='eligible'",
            payload.message_id,
        )
        if row is None:
            return
        ...
        if not row["allow_edits"]:
            await self.bot.db.execute(
                "UPDATE notice_submissions SET status='invalid',moderated_at=NOW(),moderation_reason='edits are disabled' WHERE id=$1",
                row["id"],
            )
            return
        reason = self._invalid_submission_reason(
            row, content, bool(payload.data.get("attachments"))
        )
        if reason:
            await self.bot.db.execute(
                "UPDATE notice_submissions SET status='invalid',moderated_at=NOW(),moderation_reason=$2 WHERE id=$1",
                row["id"],
                f"edited: {reason}",
            )
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        if row:
            queue = await self.bot.db.fetchrow(
                "SELECT guild_id FROM notice_queues WHERE id=$1", row["queue_id"]
            )
            await self.bot.db.execute(
                "INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,$2,'submission_withdrawn','notice_submission',$3,$4)",
                queue["guild_id"],
                row["author_id"],
            ...
            await self._alert(
                queue["guild_id"],
                text("queue.audit.withdrawn", locale, message_id=payload.message_id),
            )

    async def publish(
        self, queue_id: int, scheduled_for: datetime | None = None
    ) -> str:
        async with self._lock:
            ...
            async with self.bot.db.acquire() as conn, conn.transaction():
                ...
                submission = await conn.fetchrow(
                    f"SELECT s.* FROM notice_submissions s WHERE s.queue_id=$1 AND s.status='eligible' ORDER BY {order} FOR UPDATE SKIP LOCKED LIMIT 1",
                    queue_id,
                )
                if submission is None:
                    await conn.execute(
                        "INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,0,'notice_queue_empty','notice_queue',$2,$3)",
                        queue["guild_id"],
                        str(queue_id),
                        {"action": queue["no_entry_action"]},
                    )
                    raise ValueError("No eligible submission is available.")
                await conn.execute(
                    "UPDATE notice_submissions SET status='selected' WHERE id=$1",
                    submission["id"],
                )
                if scheduled_for:
                    await conn.execute(
                        "INSERT INTO notice_jobs(queue_id,job_type,scheduled_for,state,lease_expires_at,attempt_count) VALUES($1,'publish',$2,'running',NOW() + INTERVAL '10 minutes',1) ON CONFLICT(queue_id,job_type,scheduled_for) DO NOTHING",
                        queue_id,
                        scheduled_for,
                    )
            source = self.bot.get_channel(
                submission["source_thread_id"]
            ) or await self.bot.fetch_channel(submission["source_thread_id"])
            try:
                source_message = await source.fetch_message(
                    submission["source_message_id"]
                )
            except discord.NotFound:
                await self.bot.db.execute(
                    "UPDATE notice_submissions SET status='withdrawn', moderation_reason='source message unavailable' WHERE id=$1",
                    submission["id"],
                )
                raise ValueError("The selected submission was deleted.")
            destination = self.bot.get_channel(
                queue["publication_channel_id"]
            ) or await self.bot.fetch_channel(queue["publication_channel_id"])
            await self._validate_channel(
                queue["guild_id"], queue["publication_channel_id"], "publication"
            )
            author = (
                source_message.author.display_name
                if queue["attribution_policy"] == "display_name"
                else ""
            )
            title, content = self._render_publication(
                queue, author, source_message.content
            )
            files = [
                await attachment.to_file() for attachment in source_message.attachments
            ]
            ...
            await self.bot.db.execute(
                "UPDATE notice_submissions SET status='published',published_at=NOW(),publication_channel_id=$2,publication_thread_id=$3,reward_state=$4 WHERE id=$1",
                submission["id"],
                destination.id,
                publication.id,
                (
                    "pending"
                    if queue["reward_policy"].get("type") != "none"
                    else "not_applicable"
                ),
            )
            await self._issue_legacy_reward(
                queue, submission, source_message, publication
            )
            ...
            try:
                ...
                await self._alert(
                    queue["guild_id"],
                    f"**{queue['display_name']}** 다음 신청 스레드를 열지 못했습니다: {exc}",
                )
            if scheduled_for:
                await self.bot.db.execute(
                    "UPDATE notice_jobs SET state='completed',completed_at=NOW(),lease_expires_at=NULL WHERE queue_id=$1 AND job_type='publish' AND scheduled_for=$2",
                    queue_id,
                    scheduled_for,
                )
            return publication.mention

    async def _issue_legacy_reward(
        self, queue, submission, source_message, publication
    ):
        if not await game_enabled(self.bot.db, queue["guild_id"]):
            await self.bot.db.execute(
                "UPDATE notice_submissions SET reward_state='not_applicable' WHERE id=$1",
                submission["id"],
            )
            return
        ...
        try:
            ...
            state = (
                "granted"
                if result.get("awarded") or result.get("already_awarded")
                else ("not_applicable" if result.get("cap_reached") else "failed")
            )
            await self.bot.db.execute(
                "UPDATE notice_submissions SET reward_state=$2 WHERE id=$1",
                submission["id"],
                state,
            )
            if result.get("awarded"):
                text, public_text = self._reward_messages(
                    kind, result, source_message.author.display_name
                )
                try:
            ...
            await self.bot.db.execute(
                "UPDATE notice_submissions SET reward_state='failed' WHERE id=$1",
                submission["id"],
            )
            await self.bot.db.execute(
                "INSERT INTO admin_audit_log(guild_id,actor_id,action,target_type,target_id,after_state) VALUES($1,0,'notice_reward_failed','notice_submission',$2,$3)",
                queue["guild_id"],
                str(submission["id"]),
    ...
    def _reward_messages(
        kind: str, result: dict, recipient_name: str
    ) -> tuple[str, str]:
        ...
        if kind == "legacy_tomak":
            ...
            public_lines = [
                f"{recipient_name} 님이 이 토막상식을 게시하여 모든 비밀이 2배가 되었습니다."
            ]
            for key in SECRETS:
                ...
                public_lines.append(transition)
            return "\n".join(lines), "\n".join(public_lines)
        ...
        return (
            "게시된 기여에 대한 보상이 지급되었습니다.",
            f"{recipient_name} 님의 게시 기여에 대한 보상이 지급되었습니다.",
        )
    ...
    async def run_schedules(self):
        ...
        queues = await self.bot.db.fetch(
            "SELECT * FROM notice_queues WHERE state='active' AND cadence <> 'manual'"
        )
        for queue in queues:
            local = now.astimezone(ZoneInfo(queue["timezone"]))
            if (
                local.hour != queue["schedule_time"].hour
                or local.minute != queue["schedule_time"].minute
            ):
                continue
            if (
                queue["cadence"] == "weekly"
                and local.weekday() != queue["schedule_weekday"]
            ):
                continue
            scheduled_for = local.replace(second=0, microsecond=0).astimezone(
                timezone.utc
            )
            existing = await self.bot.db.fetchrow(
                "SELECT state FROM notice_jobs WHERE queue_id=$1 AND job_type='publish' AND scheduled_for=$2",
                queue["id"],
                scheduled_for,
            )
            ...
            try:
                ...
                if (
                    str(exc) == "No eligible submission is available."
                    and queue["no_entry_action"] == "alert_and_reopen"
                ):
                    try:
                ...
                await self._alert(
                    queue["guild_id"],
                    f"**{queue['display_name']}** 예약 게시를 처리하지 못했습니다: {exc}",
                )
                await self.bot.db.execute(
                    "INSERT INTO notice_jobs(queue_id,job_type,scheduled_for,state,attempt_count,last_error) VALUES($1,'publish',$2,'failed',1,$3) ON CONFLICT(queue_id,job_type,scheduled_for) DO UPDATE SET state='failed',attempt_count=notice_jobs.attempt_count+1,last_error=$3",
                    queue["id"],
                    scheduled_for,
                    str(exc)[:500],
                )
